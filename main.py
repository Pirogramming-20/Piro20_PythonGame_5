    ...
    if response.status_code == 200:
        ...
        for row in table.tbody.find_all('tr')[:68]:
            ...
            if len(columns) >= 2:
                ...
                title = columns[1].text.split('《')[1].split('》')[0].strip()
                movies.append((int(rank), title))
        return movies
    else:
        return []
...
def movie_rank_game():
    ...
    if movies:
      ...
      if(game_people[0] == player_name):
        ...
        betting = random.randint(1,3)
      print(f"{game_people[0]}이(가) 이번판에 건 술은 {betting} 잔입니다! {'🥃' * betting}\n")
      while True:
          time.sleep(1)
          movie1, movie2 = random.sample(movies, 2)
          correct_choice = '1' if movie1[0] < movie2[0] else '2'

          if game_people[player_turn] == player_name:
              print(f"관객수 더 높을것 같은 영화를 골라주세요? 👤👤\n\n1. 🎥 {movie1[1]}\n2. 🎥 {movie2[1]}\n")
              choice = input(f"{player_name}님 숫자 1 또는 2 를 입력하세요 : ").strip()
...
while(dur):
  ...
  print("{:>32}".format("4. 🍿 🎬 영화 순위 게임 🍿 🎬"))
  print("{:>32}".format("5. 🎲 🎲 3, 6 , 9 🎲 🎲"))
  print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
  if(game_people[0] != player_name):
  ...
  for person_alc in people_alc:
    if person_alc <= 0:
      ...
      print('='*60)
      print('='*60)
      print("{}이(가 ) 전사했습니다... 꿈나라에서는 편히 쉬시길...zzz".format(game_people[people_alc.index(person_alc)]))
      print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
